...
class TestMuranoPluginMU(api.MuranoPluginApi):
    ...
    @test(depends_on_groups=["deploy_murano_plugin"],
          groups=["test_mu_upgrade_murano_node", "murano", "test_mu_upgrade"])
    ...
    def test_mu_upgrade_murano_node(self):
        ...
        self.apply_maintenance_update_90_to_91(murano_node=True)
        self.run_ostf(['sanity', 'smoke', 'tests_platform'])

    @test(depends_on_groups=["deploy_murano_plugin_on_controller"],
          groups=["test_mu_upgrade_murano_plugin", "murano", "test_mu_upgrade"])
    @log_snapshot_after_test
