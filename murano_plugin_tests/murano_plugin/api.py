...
class MuranoPluginApi(object):
    ...
    def apply_maintenance_update_90_to_91(self):
        ...
        timestamp = self.helpers.get_last_timestamp(settings.CENTOS_REPO_URL)
        ...
        timestamp_ubuntu = self.helpers.get_last_timestamp(
            settings.UBUNTU_REPO_URL, distro='ubuntu')

        proposed = {
            'name': 'proposed',
            'section': 'main restricted',
            'uri': settings.UBUNTU_REPO_URL + '9.0-' + timestamp_ubuntu,
            'priority': 1200,
        ...
        self.helpers.update_nodes_with_key(
            settings.UBUNTU_REPO_URL, timestamp_ubuntu)
