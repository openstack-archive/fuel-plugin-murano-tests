...
class TestMuranoPluginUpdate(api.MuranoPluginApi):
    ...
    def deploy_environment_with_murano_plugin(self):
        ...
        self.helpers.create_cluster(name=self.__class__.__name__, settings={'murano': True})
    ...
    @test(depends_on_groups=["prepare_slaves_3"],
          groups=["deploy_murano_and_plugin_add_role", "deploy",
                  "murano"])
    @log_snapshot_after_test
    def deploy_murano_plugin_in_existing_environment_with_murano(self):
        ...
        Scenario:
            ...
            3. Upload plugin to master node
            4. Install plugin
            5. Activate plugin and add new node with murano-node role to the cluster
            6. Update cluster (Deploy changes)
            7. Run OSTF

        Duration 120m
        Snapshot deploy_murano_plugin_in_existing_environment_with_murano
        """
        self.check_run("deploy_murano_plugin_in_existing_environment_with_murano")
        ...
        self.env.make_snapshot("deploy_murano_plugin_in_existing_environment_with_murano",
                               is_make=True)